#!/usr/bin/env bash
# rmpc-fzf-newest.sh: Sort albums by file modification time (Date Added)

# 1. Get all files with their last modified date, sort newest first
# We use 'listallinfo' to get the Last-Modified timestamp per file
# We then group them by Artist - Album and keep the newest timestamp for each.
