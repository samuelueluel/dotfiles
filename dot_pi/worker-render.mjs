...
const count = process.env.DELEGATION_COUNT || "X";
let title = `        WORKER DELEGATION #${count} STARTED`;
while (title.length < 49) title += " ";

console.log(`\n${C_CYAN}${C_BOLD}=================================================${C_RESET}`);
console.log(`${C_CYAN}${C_BOLD}${title}${C_RESET}`);
...
rl.on('line', (line) => {
  ...
  if (ev.type === "tool_execution_start") {
    ...
    if (resultText.length > 2000) resultText = resultText.slice(0, 2000) + "… [TRUNCATED FOR PANE READABILITY]";
