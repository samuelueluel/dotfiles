...
- Explore model policy: local parents (`pi`/`beta`) pin Explore to the parent's exact model; cloud parents (`pihat`/`betahat`) pin Explore to `openai-codex/gpt-5.6-luna`. Never choose or pass a `model` for Explore; it is ignored. You may set `thinking` per task; cloud defaults to `medium` when omitted.
