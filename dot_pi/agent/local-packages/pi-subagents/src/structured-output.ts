 ...
 *
 * A caller that passes a `schema` wants an *object* back, not prose it has to
 * parse. The child gets a `StructuredOutput`
 ...
 *
 * The name matches the established tool contract, so callers that mention
 * `StructuredOutput` are still telling the truth.
 */
