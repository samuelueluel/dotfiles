...
/**
 * Recover an agent ID that a model expanded back into its source UUID. Agent
 * IDs are the first 17 UUID characters; only a unique top-level prefix match
 * is accepted, so arbitrary fuzzy strings never address an agent.
 */
export function resolveExpandedAgentId(
  ref: string,
  records: readonly AgentRecord[],
): AgentRecord | undefined {
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(ref)) return undefined;
  const matches = records.filter(record => isTopLevelAgent(record) && ref.startsWith(record.id));
...
export default function (pi: ExtensionAPI) {
  ...
  const manager = new AgentManager((record) => {
    // Owned children report only through their parent scope. Keep them out of
    // top-level lifecycle, transcript, notification, and UI channels.
  ...
  const spawnTopLevel = (piRef: any, ctxRef: any, type: string, prompt: string, options: any) => {
    ...
    // another owner and take it out of the concurrency pool with it.
   ...
   *
   * Agent IDs are intentionally short (the first 17 characters of a UUID) for
   * compact tool output. Models sometimes expand that token back into a UUID;
   * accept the expansion only when it is a valid UUID-shaped string with one
   * unique live top-level ID prefix. This repairs the common transcription
   * failure without making arbitrary fuzzy IDs addressable.
   */
  const resolveAgentRef = (ref: string): AgentRecord | undefined => {
    const byId = manager.getRecord(ref);
    if (byId) return byId;

    const expanded = resolveExpandedAgentId(ref, manager.listAgents());
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  const agentTool = defineTool({
    ...
    parameters: Type.Object({
      ...
      max_turns: Type.Optional(
        Type.Number({
          description: "Optional hard budget in agentic turns. At the limit the child is told to wrap up and its output may be partial; this is not a tool-call budget. Omit for unlimited (default).",
          minimum: 1,
        }),
      ),
      ...
      if (params.resume) {
        ...
        if (runInBackground) {
          ...
          return textResult(
            ...
            `Agent ID: ${id}\n` +
            `ID note: copy this exact 17-character token; do not expand it into a full UUID.\n` +
            (record.handle ? `Handle (also accepted): ${record.handle}\n` : "") +
            `Type: ${existing.type}\n` +
  ...
  registerToolReportingUsage(defineTool({
    ...
    description:
      "Check status and retrieve a background agent's full result — its completion notification carries only a preview. Use the exact 17-character agent ID returned by Agent, or its handle; copy IDs verbatim and do not expand them into a full UUID.",
    promptSnippet: "Check status and retrieve results from a background agent",
    parameters: Type.Object({
      agent_id: Type.String({
        description: "The exact 17-character agent ID to check; copy it verbatim, do not expand it into a full UUID. The agent's handle also works — its `name` if you gave it one, otherwise its type (`explore`, `explore-2`).",
      }),
      wait: Type.Optional(
        Type.Boolean({
  ...
  registerToolReportingUsage(defineTool({
    ...
    description:
      ...
      "and be injected into its conversation, allowing you to redirect its work mid-run. Only works on running agents. Use the exact 17-character ID returned by Agent, or its handle; copy IDs verbatim and do not expand them into a full UUID.",
    promptSnippet: "Send a steering message to redirect a running background agent",
    parameters: Type.Object({
      agent_id: Type.String({
        description: "The exact 17-character agent ID to steer (must be currently running); copy it verbatim, do not expand it into a full UUID. The agent's handle also works — its `name` if you gave it one, otherwise its type (`explore`, `explore-2`).",
      }),
      message: Type.String({
...
Write the file using the write tool. Only write the file, nothing else.`;
  ...
  function snapshotSettings() {
    return {
      ...
      // Legacy field retained as an explicit false so an old subagents.json
      // cannot resurrect the retired workflow feature through a settings save.
