...
export interface SubagentsSettings {
  ...
  /**
   * Legacy compatibility field. Scripted workflows are retired by the
   * extension and cannot be re-enabled by configuration; the field remains
   * optional so older subagents.json files continue to parse.
   */
...
export function loadSettings(cwd: string = process.cwd()): SubagentsSettings {
  const merged = { ...readSettingsFile(globalPath()), ...readSettingsFile(projectPath(cwd)) };
  const envMax = parseInt(process.env.PI_SUBAGENTS_MAX_CONCURRENT || "", 10);
  if (!isNaN(envMax) && envMax >= 1 && envMax <= MAX_CONCURRENT_CEILING) {
    merged.maxConcurrent = envMax;
  }
  return merged;
}
