...
5. **Read sources after discovery:** Neural RAG returns candidate chunks with provenance (`path#chunk_id`) and, when indexed, `chunk_hash`. When a specific hit needs more context, call `turbovault_read_passage` with the returned path, chunk ID, and hash for bounded same-section neighbors; it reopens stored index text without new ranking or score inheritance. Then use `turbovault_read_note` for current Markdown source context before consequential claims. Sparse-only hits may not have a chunk ID; read their note directly. Follow [Obsidian evidence integrity](../obsidian-evidence-integrity/SKILL.md) for claim-level attribution and note links.
...
- If reporting information from vault notes to Samuel, load [Obsidian evidence integrity](../obsidian-evidence-integrity/SKILL.md); attribute each material claim to its actual source note and section.
