...
| Audit schema exposes `check_mode`, or audit returns `CHECKER_UNAVAILABLE` / `CHECKER_SKIPPED` | Legacy checker-based audit release is still deployed | Stop without spending the repair rerun. Reinstall the current pinned fork release, restart `zotero-mcp.service`, and confirm the live schema is deterministic-only. |
