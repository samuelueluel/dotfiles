...
| Audit returns `QUOTE_NOT_FOUND`, or failure codes conflict with displayed scores | The current audit separates rejected-candidate diagnostics from retained evidence; a conflict usually indicates a stale service, mixed deployment, or response-contract drift | Verify the running package and restart the service before retrying. Preserve the deterministic result, use one targeted original-source read within the repair budget if needed, and do not infer fabrication. |
