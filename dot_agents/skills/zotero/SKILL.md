...
### 3. Searching & Retrieval (RAG & Citation Graph)
- **Tool matrix:** `semantic_search` = conceptual topic search (hybrid BM25+RRF + cross-encoder rerank run automatically; `collection=<8-char KEY>` scopes to a collection). `get_collection_hubs` = find foundational hub papers cited across a collection. `get_paper_lineage` = ancestor citations & descendant papers citing an item. `find_connected_papers` = bibliographic coupling (shared literature overlap). `advanced_search` = structured metadata filters. `search_items` = substring title/creator match. `search_by_citation_key` / `search_by_tag` = exact citekey / tag lookup.
