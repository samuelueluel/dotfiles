...
- Samuel permits the active `pihat` conversation model to receive normalized, OCR, and visual artifacts, just as it receives Zotero-MCP output. This is artifact interaction, not cloud preprocessing. State this plainly when `pihat` is active.
...
A known local or known cloud Pi route may use the fixed-operation bridge. Unknown provider/endpoint identity still fails closed. A cloud route may receive artifacts through the bridge, but may not reach the workspace through alternate filesystem tools.

The active conversation route does not change the enrichment route. `document_analysis_enrich` always invokes the local MinerU executable and loopback VLM only. A cloud `pihat` model may read the result returned by that operation; the bridge applies no custom output truncation, while model and transport context limits still apply.
...
- Enrichment may return its evidence to the active `pihat` model, but no cloud service may perform the preprocessing itself.
...
Phase 2 OCR sends image inputs and PDF pages with empty or short native text (under 80 characters by default) to the installed local MinerU adapter. Whenever MinerU runs, its enabled formula detection and formula-to-LaTeX recognition may emit detected equations as LaTeX in the OCR Markdown/content-list evidence. Strong native-text PDF pages skip MinerU entirely: `ocr: not_needed` means ordinary OCR was skipped, not that formula fidelity was verified. Do not force full OCR solely to improve LaTeX unless the native page shows an actual defect. OCR evidence remains separate and never replaces native text. Visual evidence records page, region, type, transcription, observation, interpretation, and confidence fields; numeric visual text is suppressed until independently verified.
