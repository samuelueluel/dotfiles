...
**Load this file when** resolving retrieval or audit errors, recovering wedged services, operating with Zotero Desktop closed, or executing SQLite database diagnostics.
...
| Audit schema exposes `check_mode`, or audit returns `CHECKER_UNAVAILABLE` / `CHECKER_SKIPPED` | Legacy checker-based audit release is still deployed | Stop the requested audit without spending the repair rerun and report the capability problem. If Samuel authorizes service repair, reinstall the current pinned fork release, restart `zotero-mcp.service`, and confirm the live schema is deterministic-only. |
| Audit returns `QUOTE_NOT_FOUND` | The submitted quote is not an exact normalized substring; common causes are reconstructed column text, omitted parentheticals/layout labels, OCR repair, or changed line-ending hyphenation | Reuse the exact successful query and copy a shorter exact contiguous source substring. Permit one targeted page/sidecar read within the repair budget. Do not infer that the paper lacks the claim. |
| A retained evidence preview omits its quote, or retained positive-Rerank evidence reports conflicting score diagnostics | Stale service, mixed deployment, or response-contract drift | Verify the running package and restart the service before retrying. Preserve returned provenance fields and do not infer fabrication or source-level contradiction. |
